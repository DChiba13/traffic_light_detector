...
int main(int argc, char **argv)
{
  ...
  getFiles(argv[1],".png",files_png);
    ...
    while (true)
    {
      ...
      cv::dilate(bin_img_red, bin_img_red, kernel);
      cv::dilate(bin_img_green, bin_img_green, kernel);
      ...
      for (int label = 0; label < num_labels_green; label++)
      {
        ...
        // ピクセル数、アスペクト比を見る
        pixel_num = width * height;
        // std::cout << "pixel_num_green : " << pixel_num << std::endl;
        aspect_ratio = ((double)width) / ((double)height);
        // std::cout << "aspect_ratio_green : " << aspect_ratio << std::endl;
        if(pixel_num<MIN_PIX_NUM || pixel_num>MAX_PIX_NUM)
        {
          continue;
        }
      ...
      extractYellowInBlob(camera_img, bin_img_red, num_labels_red, red_width, red_height, red_left, red_top, true, top_region);
      extractYellowInBlob(camera_img, bin_img_green, num_labels_green, green_width, green_height, green_left, green_top, false,top_region);
