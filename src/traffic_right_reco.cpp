...
void createCandidateArea(Mat &src, const Mat &labeled, const Mat &stats, const Mat &centroids, vector<int> &left, vector<int> &top, vector<int> &width, vector<int> &height, int num_labels, string color_type)
{
  for (int label = 1; label < num_labels; label++)
  {
    ...
    if(color_type == "red") cv::rectangle(src, cv::Rect(left[label], top[label], width[label], height[label]), cv::Scalar(255,0,255), 2);
    else if(color_type == "green") cv::rectangle(src, cv::Rect(left[label], top[label], width[label], height[label]), cv::Scalar(255,255,0), 2);
  }
}
...
int main(int argc, char **argv)
{
  ...
  while (true)
  {
    ...
    color_type = "red";
    createCandidateArea(top_region, labeled_red, stats_red, centroids_red, red_left, red_top, red_width, red_height, num_labels_red, color_type);
